...
CREATE TABLE PRODUCTS (
    ...
    CATEGORIZEDBY INT NOT NULL,
    ROWVERSION NUMBER(20) DEFAULT 0 NOT NULL,

    CONSTRAINT FK_PRODUCTS_CREATEDBY FOREIGN KEY (CREATEDBY) REFERENCES USERS(KEY),
    CONSTRAINT FK_PRODUCTS_MODIFIEDBY FOREIGN KEY (MODIFIEDBY) REFERENCES USERS(KEY),
    CONSTRAINT FK_PRODUCTS_SUPPLIEDBY FOREIGN KEY (SUPPLIEDBY) REFERENCES SUPPLIERS(KEY),
    CONSTRAINT FK_PRODUCTS_CATEGORIZEDBY FOREIGN KEY (CATEGORIZEDBY) REFERENCES CATEGORIES(KEY)
);

-- Secuencia para la columna KEY
CREATE SEQUENCE PRODUCTKEYSEQ START WITH 1 INCREMENT BY 1 NOCACHE NOCYCLE;
...
BEGIN
    IF :NEW.KEY IS NULL THEN
        :NEW.KEY := PRODUCTKEYSEQ.NEXTVAL;
    END IF;
END;
/

-- Trigger BEFORE UPDATE para incrementar el ROWVERSION y actualizar la fecha de modificación
CREATE OR REPLACE TRIGGER PRODUCTS_BU
